...
def create_embeddings(text: str | list[str]) -> list[float]:
    response = client.embeddings.create(
        model="text-embedding-3-small",
        input=text
    )
...
first_article_vector = articles[0]["embedding"]


def get_similarity_scores():
    try:
        for i in range(len(articles)):
            article_vector = articles[i]["embedding"]
            sim = cosine_similarity(first_article_vector, article_vector)
